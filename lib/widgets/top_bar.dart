  ...
  Widget build(BuildContext context) {
    ...
    return showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Персональные данные', textAlign: TextAlign.center,),
              ...
              return Column(
                ...
                children: <Widget>[
                  TextField(
                    ...
                    decoration: InputDecoration(hintText: nameController.text.trim().isEmpty ? 'Имя не указано' : null),
                  ),
                  TextField(
                    ...
                    decoration: InputDecoration(hintText: phoneController.text.trim().isEmpty ? 'Номер не указан' : null),
                  ),
                ],
              );
            } else{
                ...
                onPressed: () async{
                  await _userData.then((e){
                    if(e.docs[0].data()['id_doctor'] != null){
                      FirebaseFirestore.instance.collection('doctors/' + e.docs[0].data()['id_category'].toString() + '/' + e.docs[0].data()['id_category'].toString()).doc(e.docs[0].data()['id_doctor'].toString())
                      .update({
                        'name': nameController.text.trim(),
                        'phone': phoneController.text.trim()
                      });
                    } else if(e.docs[0].data()['id_user'] != null){
                      FirebaseFirestore.instance.collection('users').doc(e.docs[0].data()['id_user'].toString())
                      .update({
                        'name': nameController.text.trim(),
                        'phone': phoneController.text.trim()
                      });
